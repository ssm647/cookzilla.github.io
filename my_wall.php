...
$sql = "INSERT INTO recipe (recipe_username, recipe_title, discription, recipe_image, recipe_time) VALUES ('$username','$title','$description', '$target_file', '$inm')";
...
<div class="w3-panel w3-card-2 w3-white">
		...
		<button name='button1' type='button1' type="button" class="btn btn-default btn-sm">
       ...
       <?php 
		...
		$result1 = $mysqli->query("SELECT count(recipe_id) as count1 FROM recipe WHERE recipe_username='$username'");
		...
		$result2 = $mysqli->query("SELECT count(following_name) as count2 FROM following WHERE username='$username' AND following_name!='$username' GROUP BY username");
		...
		$result3 = $mysqli->query("SELECT count(username) as count3 FROM following WHERE following_name='$username' AND username!='$username' GROUP BY following_name");
